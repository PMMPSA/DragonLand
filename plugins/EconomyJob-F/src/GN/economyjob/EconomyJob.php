 ...
 * Copyright (C) 2013-2015  Ghast_Noob <nhangaming125@gmail.com>
 *
...
namespace Ghast_Noob\economyjob;
		...
		switch(array_shift($params)){
			case "join":
				...
				if($this->player->exists($sender->getName())){
					$sender->sendMessage("Bạn đã từng tham gia một công việc.");
				}else{
					$job = array_shift($params);
					if(trim($job) === ""){
						$sender->sendMessage("Sử dụng: /job join <tên nghề>");
						break;
					}
					if($this->jobs->exists($job)){
						$this->player->set($sender->getName(), $job);
						$sender->sendMessage("Bạn đã tham gia làm nghề \"$job\"");
					}else{
						$sender->sendMessage("Không có nghề nào tên là \"$job\"");
					}
				}
				break;
			case "retire":
				...
				if($this->player->exists($sender->getName())){
					...
					$sender->sendMessage("Bạn đã nghỉ làm \"$job\"");
				}else{
					$sender->sendMessage("Bạn chưa từng tham gia công việc nào");
				}
				break;
			case "list":
			...
			case "me":
				...
				if($this->player->exists($sender->getName())){
					$sender->sendMessage("Công việc của bạn : ".$this->player->get($sender->getName()));
				}else{
					$sender->sendMessage("Bạn chưa từng tham gia công việc nào.");
				}
				break;
